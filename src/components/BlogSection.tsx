      ...
      <section id="blog" ref={sectionRef} className="py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12">
            <p className="text-accent text-sm font-medium mb-2 flex items-center gap-2"><BookOpen className="w-4 h-4" /> Blog & Articles</p>
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground">Thoughts & <em className="font-normal italic">Insights</em></h2>
            <p className="mt-4 text-lg text-muted-foreground max-w-xl">I write about AI, machine learning, and software development.</p>
          </motion.div>
          <div className="grid md:grid-cols-2 gap-6 max-w-2xl">
            <a href={blogSiteUrl} target="_blank" rel="noopener noreferrer" className="group block">
              <div onMouseMove={handleSpotlight} className="spotlight-card p-6 border border-border bg-card hover:border-accent/30 transition-all duration-300">
                <Sparkles className="w-5 h-5 text-accent mb-3" />
                <h3 className="font-bold text-foreground mb-2">Dinmay's Blog</h3>
                <p className="text-sm text-muted-foreground mb-4">Visit my personal blog for in-depth articles.</p>
                <span className="text-accent text-sm font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">Visit Blog <ArrowRight className="w-4 h-4" /></span>
              </div>
            </a>
            <a href={mediumUrl} target="_blank" rel="noopener noreferrer" className="group block">
              <div onMouseMove={handleSpotlight} className="spotlight-card p-6 border border-border bg-card hover:border-accent/30 transition-all duration-300">
                ...
                <h3 className="font-bold text-foreground mb-2">View on Medium</h3>
                <p className="text-sm text-muted-foreground mb-4">Read all articles on Medium.</p>
                <span className="text-accent text-sm font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">Open Medium <ArrowRight className="w-4 h-4" /></span>
              </div>
            </a>
          </div>
        </div>
  ...
  if (loading) {
    return (
      <section id="blog" ref={sectionRef} className="py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-16">
          <p className="text-accent text-sm font-medium mb-2 flex items-center gap-2"><BookOpen className="w-4 h-4" /> Blog & Articles</p>
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground mb-8">Thoughts & <em className="font-normal italic">Insights</em></h2>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12">
          ...
          <div className="flex items-center justify-between mt-4">
            <p className="text-lg text-muted-foreground max-w-xl">Sharing knowledge about AI, machine learning, and software development.</p>
            <div className="hidden md:flex items-center gap-3">
              <span className="flex items-center gap-2 text-xs text-green-400">
                <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />Live from Medium
              </span>
        ...
        {posts.length > 0 && (
          <motion.a href={posts[0].link} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="group block mb-8">
            <div onMouseMove={handleSpotlight} className="spotlight-card border border-border bg-card hover:border-accent/30 transition-all duration-300 grid md:grid-cols-2 overflow-hidden">
              ...
              <div className="p-6 md:p-8 flex flex-col justify-center">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  {posts[0].categories.slice(0, 2).map((cat, idx) => (
                    <span key={idx} className={`text-xs px-2 py-1 rounded-sm font-medium ${getCategoryColor(cat)}`}>{cat}</span>
                  ))}
                  <span className="text-xs px-2 py-1 rounded-sm font-medium bg-accent/20 text-accent">Latest</span>
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-foreground mb-3 group-hover:text-accent transition-colors leading-snug">{posts[0].title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed mb-4">{posts[0].description}</p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {posts.slice(1).map((post, index) => (
              <motion.a key={post.id} href={post.link} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: (index + 1) * 0.1 }} className="group block">
                <div onMouseMove={handleSpotlight} className="spotlight-card h-full border border-border bg-card hover:border-accent/30 transition-all duration-300 flex flex-col overflow-hidden">
                  <div className="relative h-44 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex gap-1">
                      {post.categories.slice(0, 1).map((cat, idx) => (
                        <span key={idx} className={`text-[10px] px-2 py-0.5 rounded-sm font-medium ${getCategoryColor(cat)}`}>{cat}</span>
                      ))}
                    </div>
                  </div>
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h3 className="text-base font-bold text-foreground mb-2 group-hover:text-accent transition-colors leading-snug">{post.title}</h3>
                    <p className="text-sm text-muted-foreground leading-relaxed flex-1 mb-4">{post.description}</p>
                    <span className="text-accent text-xs font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">Read on Medium <ExternalLink className="w-3 h-3" /></span>
                  </div>
                </div>
              </motion.a>
            ))}
        ...
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.5 }} className="mt-10 flex items-center justify-center gap-6">
          <a href={mediumUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-accent font-semibold text-sm hover:gap-3 transition-all">
            View All on Medium <ArrowRight className="w-4 h-4" />
          </a>
          <a href={blogSiteUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-muted-foreground font-semibold text-sm hover:text-foreground hover:gap-3 transition-all">
